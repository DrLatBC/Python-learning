...
# Game add/remove
# --------------------------------------
def ensure_week_bucket(data, year, week):
    seasons = data.setdefault("seasons", {})
    sy = seasons.setdefault(str(year), {})
    weeks = sy.setdefault("weeks", {})
    wk = weeks.setdefault(str(week), {})
    games = wk.setdefault("games", [])
    return games

def generate_game_id(games, year, week):
    existing_ids = [g.get("game_id", "") for g in games]
    gnums = [
        int(gid.split("G")[-1])
        for gid in existing_ids
        if gid.startswith(f"S{year}W{week}G") and "G" in gid
    ]
    next_gnum = max(gnums, default=0) + 1
    return f"S{year}W{week}G{next_gnum}"
...
def add_game(master_file, new_game_file, year, week):
    ...
    if not {"team_a", "team_b"} <= set(new_game_data.keys()):
        raise ValueError("new_game.json must contain 'team_a' and 'team_b' objects")

    # 🔥 Remove 'score' at the source if present
    ...
    # Verify (abort on issues)
    issues = verify_game(new_game, game_id)
    if issues:
        out = "\n - " + "\n - ".join(issues)
        raise ValueError(f"Verification FAILED:{out}")

    # Loud warnings (non-fatal)
    for w in warnings:
        print(w)

    # Dupe check: same teams with identical starters packages (basic name match)
    for g in games:
    ...
    print(f"✅ Game {game_id} added for Week {week}, {year}: {team_a['name']} vs {team_b['name']} → saved to {master_file}")

def preview_remove(games, game_id, year, week):
    # show what will be removed and how IDs shift
    idx = None
...
def remove_game(master_file, year, week, game_id, non_interactive=False):
    data = load_json(master_file)
    games = ensure_week_bucket(data, year, week)

    # Preview
    preview_remove(games, game_id, year, week)

    if non_interactive:
        confirm = "y"
    else:
        confirm = input("Type 'y' to confirm removal and reindexing, anything else to cancel: ").strip().lower()

    if confirm == "y":
        apply_remove(master_file, data, year, week, game_id)
    else:
...
def usage():
    print("Usage:")
    print("  Add:    python add_remove_game.py add <master_file.json> <new_game.json> <year> <week>")
    print("  Remove: python add_remove_game.py remove <master_file.json> <year> <week> <game_id> [--yes]")
    sys.exit(1)
...
if __name__ == "__main__":
    ...
    if cmd == "add":
        if len(sys.argv) != 6:
            usage()
        _, _, master_file, new_game_file, year, week = sys.argv
        try:
        ...
        # allow optional --yes to skip prompt
        ...
        week = sys.argv[4]
        game_id = sys.argv[5]
        non_interactive = (len(sys.argv) == 7 and sys.argv[6] == "--yes")
        try:
            remove_game(master_file, year, week, game_id, non_interactive=non_interactive)
        except Exception as e:
            print(f"❌ Error: {e}")
            sys.exit(1)
    else:
